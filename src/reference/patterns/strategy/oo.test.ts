import { Product, NoDiscountStrategy, SeasonalDiscountStrategy, BulkDiscountStrategy } from './oo';
...
describe('Productクラスにおける割引戦略のテスト', () => {
  it('割引なしの場合は元の価格を返すこと', () => {
    const product = new Product('ノートパソコン', 100000, new NoDiscountStrategy());
    expect(product.getDiscountedPrice()).toBe(100000);
  });

  it('季節割引（20%オフ）を適用した場合は20%引きの価格を返すこと', () => {
    const product = new Product('ノートパソコン', 100000, new SeasonalDiscountStrategy());
    // 100000 の 20%オフ → 80000
    expect(product.getDiscountedPrice()).toBe(80000);
  });

  it('まとめ買い割引（500円引き）を適用した場合は500円値引きされた価格を返すこと', () => {
    const product = new Product('ノートパソコン', 100000, new BulkDiscountStrategy());
    // 100000 - 500 → 99500
